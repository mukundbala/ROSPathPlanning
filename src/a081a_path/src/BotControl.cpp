...
BotControl::BotControl(ros::NodeHandle& nh)
{
  ...
  start_correction_pub_=nh_.advertise<std_msgs::Bool>("/start_corr",1);
...
void BotControl::controlPub()
{
  ...
  I_heading += dt*error_heading_;
  I_pos += dt*error_pos_;
  
  D_heading = error_heading_prev_ - error_heading_;
  D_pos = error_pos_prev_ - error_pos_;
...
void BotControl::spin()
{
  ...
  // sleep at the start to wait for all the windows to load!
  int slpTime=5;
  for (;slpTime>0;--slpTime){
    ROS_INFO("%d",slpTime);
    loop_rate.sleep();
  }//go to sleep :)
  ...
  cmd.angular.y = 0;
  cmd.angular.z = 0;
  control_pub_.publish(cmd); //publish to kill the robot motion
  
  checker.data=true;
  start_correction_pub_.publish(checker);
